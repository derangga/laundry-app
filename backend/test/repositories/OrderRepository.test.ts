...
import { OrderRepository, OrderInsertData, OrderFilterOptions } from '@repositories/OrderRepository'
import {
  Order,
  OrderId,
  OrderStatus,
...
const createMockOrderRepo = (orders: Order[] = []) =>
  Layer.succeed(OrderRepository, {
    findById: (id: OrderId) => Effect.succeed(Option.fromNullable(orders.find((o) => o.id === id))),
    findByOrderNumber: (orderNumber: string) =>
      Effect.succeed(Option.fromNullable(orders.find((o) => o.order_number === orderNumber))),
    ...
    updateTotalPrice: (_id: OrderId, _totalPrice: number) => Effect.succeed(void 0),
