...
const createMockCustomerRepo = (
  ...
  Layer.succeed(CustomerRepository, {
    findById: (id: CustomerId) =>
      Effect.succeed(
        Option.fromNullable(customers.find((c) => c.id === id))
      ),
    findByPhone: (phone: string) =>
      Effect.succeed(
        Option.fromNullable(customers.find((c) => c.phone === phone))
      ),
...
describe("CustomerRepository", () => {
  ...
  describe("update", () => {
    ...
    it("should return None when customer does not exist", async () => {
      ...
      const program = Effect.gen(function* () {
        const repo = yield* CustomerRepository
        return yield* repo.update("nonexistent" as CustomerId, { name: "Test" } as UpdateCustomerInput)
      })
