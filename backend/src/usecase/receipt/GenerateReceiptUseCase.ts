...
export const generateReceiptUseCaseImpl = Effect.gen(function* () {
  ...
  const execute = Effect.fn('GenerateReceiptUseCase.execute')(function* (orderId: OrderId) {
    // 1. Fetch order
...
export class GenerateReceiptUseCase extends Effect.Service<GenerateReceiptUseCase>()(
  'GenerateReceiptUseCase',
  {
    accessors: true,
    effect: generateReceiptUseCaseImpl,
    dependencies: [
